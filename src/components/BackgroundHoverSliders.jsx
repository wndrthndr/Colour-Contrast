...
export default function BackgroundHoverSliders({ bgHover, setBgHover }) {
  const { r, g, b } = hexToRgb(bgHover);

  const [R, setR] = useState(r);
  const [G, setG] = useState(g);
  const [B, setB] = useState(b);
  const [hex, setHex] = useState(bgHover);
...
<div className="text-sm mb-1" style={{ color: "inherit" }}>
  ...
  return (
    <div className="panel-thick">
      <div className="font-bold text-sm mb-3">Background Hover</div>
